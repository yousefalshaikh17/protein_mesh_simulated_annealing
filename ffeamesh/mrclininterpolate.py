...
class MRCImage():
    ...
    def __init__(self, mrc):
        ...
        self._offset_x = self.dx/2.0
        self._offset_y = self.dy/2.0
        self._offset_z = self.dz/2.0

        self._inner_size_x = self.dx*(self._file.header.nx-1)
        self._inner_size_y = self.dy*(self._file.header.ny-1)
        self._inner_size_z = self.dz*(self._file.header.nz-1)
    ...
    def test_inner_coords(self, x, y, z):
        ...
        if y < 0.0:
            raise ValueError("y {message}")
        if z < 0.0:
            raise ValueError("z {message}")

        message = "coordinate above intercept calculation range"
        if x >= self._inner_size_x:
            raise ValueError("x {message}")
        if y >= self._inner_size_y:
            raise ValueError("y {message}")
        if z >= self._inner_size_z:
            raise ValueError("z {message}")
    ...
    def to_coords(self, image_x, image_y, image_z):
        ...
        """
        offset_x = image_x - self._offset_x
        offset_y = image_y - self._offset_y
        offset_z = image_z - self._offset_z
...
#         self._inner_size_x = self.dx*(self.nx-1)
#         self._inner_size_y = self.dy*(self.ny-1)
#         self._inner_size_z = self.dz*(self.nz-1)
